...
def test_settings_with_env_vars(monkeypatch):
    ...
    monkeypatch.setenv("PLAID_CLIENT_ID", "client_123")
    monkeypatch.setenv("PLAID_SECRET", "secret_abc")
    ...
    assert settings.PLAID_CLIENT_ID == "client_123"
    assert settings.PLAID_SECRET == "secret_abc"
